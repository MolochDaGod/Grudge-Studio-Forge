...
*   **Grudge Studio Puter Auth:** The toolbar's `<UserMenu>` triggers a Puter SDK popup sign-in (`artifacts/game-forge/src/lib/puterSdk.ts` lazy-loads `js.puter.com/v2/`), then exchanges the access token at `POST /api/auth/puter/exchange`. The server verifies the token with `https://api.puter.com/whoami` (server-to-server, never trusts the client's claimed identity), upserts the **shared** `users` table by `puter_uuid`, looks up the upstream Grudge ID in the **shared** `grudge_accounts` table, mints an ephemeral `GRUDGE-<ms>-<HEX>` id when none exists, and inserts a row in the **shared** `sessions` table. Cookie value is `<session_id>.<HMAC-SHA256(JWT_SECRET, session_id)>` — HttpOnly, SameSite=Lax, Secure in prod. Forge owns one tiny linking table `forge_session_links(grudge_id PK, user_id FK→users.id)` provisioned at boot via `runForgeMigrations()` with `CREATE TABLE IF NOT EXISTS` (we deliberately do NOT add the shared tables to drizzle so `drizzle push` can never touch them). Endpoints: `GET /api/auth/config`, `GET /api/auth/me`, `POST /api/auth/puter/exchange`, `POST /api/auth/logout`, `GET /api/auth/whoami`. Client store: `src/store/auth.ts`. Bootstrap: `bootstrapAuth()` runs once on `App` mount.
