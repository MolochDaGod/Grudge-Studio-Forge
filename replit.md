...
*   **Deathmatch:** First-to-N-kills mode with built-in player/enemy/gamemode JS behaviors shipped as static strings in `src/lib/deathmatchBehaviors.ts`. Behaviors are attached via a `behavior?: "player-deathmatch"|"enemy-deathmatch"|"gamemode-deathmatch"|"spawnpoint"` field on `SceneEntity` and run alongside any user `scriptId` by `ScriptedEntities`. AI movement uses Yuka (`SteeringEntity` + `SeekBehavior`) accessed via `ctx.yuka`. The HUD (`src/editor/PlayHUD.tsx`) is a DOM overlay shown only in play mode when `environment.gameMode === "deathmatch"`. Three starter templates ship in `src/lib/sceneTemplates.ts`: `dm-cyberpunk`, `dm-encampment`, `dm-deserttown`, each backed by a builtin GLB map (`/builtin/map-{cyberpunk,encampment,deserttown}.glb`). Tunable env knobs `scoreLimit` (default 10) and `respawnDelay` (default 5s) are seeded into per-entity `ctx.state` at start time.
