...
*   **Per-model yaw offset + per-race locomotion clips:** `BUILTIN_MODEL_YAW_OFFSETS` and `BUILTIN_MODEL_CLIPS` in `artifacts/game-forge/src/lib/builtinModels.ts` cover the toon-rts character pack. Every `race:*` key gets a `+π` yaw offset (the source GLBs face +Z, three.js convention is -Z) which `EntityRenderer.LoadedModel` applies on a child group inside `groupRef` (so bone animations / selection / picking still work and the physics body keeps canonical yaw — `ModelComponent.yawOffset` per-entity overrides the registry default). `CameraControllers` (TPS + FPS) and the `enemy-rpg` behavior write `idle | walk | run | attack` into `window.__agentClips` from the `BUILTIN_MODEL_CLIPS[race:<id>]` table; `LoadedModel` reads the same bridge and crossfades over 0.2s. The toon-rts character GLBs ship with **zero baked animations** on every rig and the manifest's `animationsweapons/male_locomotion/` packs return 404, so `LoadedModel` calls `synthesizeBipedClips(gltf.scene)` from `lib/proceduralBipedAnimations.ts` whenever a GLB has no clips AND its rig matches the Max biped naming (`Bip001 Pelvis / R UpperArm / L Thigh`). The synthesizer captures each animated bone's REST quaternion + position once per source scene (cached in a WeakMap keyed by `gltf.scene`), then builds absolute `restQ * deltaQ` tracks for `idle / walk / run / attack` — sagittal-plane swings on local Z for thighs/calves/upper-arms/forearms, opposite-side arm/leg phasing, sub-second cycle on run, and a triangle-wave overhand strike on attack. Clip names (`PROCEDURAL_BIPED_CLIP_NAMES`) are mirrored verbatim in `BUILTIN_MODEL_CLIPS` and the inline `RACE_CLIPS` in `enemy-rpg`; an exact-name parity drift test enforces both tables stay aligned. When the upstream pack eventually re-exports real locomotion clips into the GLBs, the `gltf.animations.length > 0` check makes the synthesizer a silent no-op and the same names resolve to the GLB-baked clips instead.
*   **2D UI Editor tab (`ui-screen` ViewportTab):** New `ViewportTabKind` `"ui-screen"` opens a `UIEditorSurface` (lazy-loaded transient surface, dedupe key `ui-screen:<screenId>`) for designing HUD overlays from the Grudge Warlords stone+gold mockups. Source of truth is `useUIScreens` (`store/uiScreens.ts`) — a separate zustand+persist store keyed `grudge.ui.screens.v1` so screen edits don't trigger 60fps scene-graph re-renders. `ProjectKey = number|"global"` lets screens be authored before any project is opened. Schema lives in `lib/uiScreen/{types,defaults,theme,widgets,exportHtml}.ts`: flat `Widget[]` (panel | text | bar | button | circle-button | hotbar | image) with absolute design-canvas px coords, `WIDGET_DEFAULTS` factory, and a single `GRUDGE_THEME_CSS` string used by both the live editor (via `ensureGrudgeThemeInjected`) and the exported HTML — what-you-see-is-what-you-export. `ScreenView` renderer is shared between editor canvas and exporter; the surface overlays it with selection/drag/resize handles (4 px snap, Delete/Esc keys). AI tools at `ai/tools/ui/` (`ui_list_screens`, `ui_get_screen`, `ui_list_widget_types`, `ui_create_screen`, `ui_rename_screen`, `ui_delete_screen`, `ui_add_widget`, `ui_update_widget`, `ui_remove_widget`, `ui_export_html`) drive the same store; `ui_create_screen` fires a `gameforge:openUIScreen` window event that `ViewportTabBar` listens for, so the model can spawn screens without importing tab internals. All mutating tools registered in `aiAuditLog.MUTATING_TOOLS`; `ui_delete_screen` and `ui_remove_widget` are in `DESTRUCTIVE_TOOLS` for confirm-gating. PR-1 export is raw HTML/CSS only — no scene-runtime integration.
*   **In-browser asset ingestion (drag-drop + ZIP packs):** `lib/fileKind.ts` (dependency-free classifier; `classifyDroppedFile` + `isModelKind`) keeps three.js out of the initial bundle; `lib/converters.ts` lazy-loads three.js loaders to transcode OBJ/FBX/STL → GLB via a shared `objectToGlbFile` (binary, embedded images, embedded animation clips) + `standardizeMaterials` (keeps existing PBR materials/texture maps, else brand-gold default). FBX animation clips ride through GLTFExporter's `animations` option. `lib/zipImport.ts` (`extractZipAssets`, uses `fflate.unzipSync`) extracts `.zip` asset packs entirely in memory — skips dirs/macOS junk/nested zips, guards entry-count (5000) + total-bytes (1.5 GB) against zip bombs. `AssetDropZone.tsx` routes each kind: glb/gltf upload raw, obj/fbx/stl convert, image/audio upload, zip extracts then re-routes each entry through a `routeRef` (avoids useCallback self-dependency). `ModelSurface.tsx` previews obj/fbx/stl by converting on open. R2 (`api-server/src/lib/r2Storage.ts`) is healthy and unchanged.
